...
export class MachinaVault {
    ...
    static async create(config) {
        ...
        const vaultInfo = await vault.http.post("/vault");
    ...
    static async connect(config) {
        const vault = new MachinaVault(config);
        await vault.authenticate();
        const vaultInfo = await vault.http.get("/vault");
        vault.vaultId = vaultInfo.id;
        return vault;
    }
    ...
    async info() {
        this.ensureConnected();
        return this.http.get(`/vault/${this.vaultId}`);
    }
    /**
     * Check a counterparty address for reputation, sanctions, and risk.
     */
    async checkCounterparty(address) {
        this.ensureConnected();
        return this.http.get(`/vault/${this.vaultId}/counterparty/${address}`);
    }
    /**
    ...
    async authenticatePasskey() {
        if (typeof globalThis.navigator === "undefined" || !navigator.credentials) {
            throw new VaultError("WebAuthn is not available in this environment", "WEBAUTHN_UNAVAILABLE", 0, {});
        }
        // Request a challenge from the server
        const challenge = await this.http.post("/auth/passkey/challenge");
        ...
        if (!credential) {
            throw new VaultError("Passkey authentication was cancelled", "PASSKEY_CANCELLED", 0, {});
        }
    ...
    validateApiKey(key) {
        if (!key || key.length < 16) {
            throw new VaultError("Invalid API key format: key must be at least 16 characters", "INVALID_API_KEY", 0, {});
        }
    }
    ...
    ensureConnected() {
        if (!this.vaultId) {
            throw new VaultError("Vault is not connected. Call MachinaVault.create() or MachinaVault.connect() first.", "NOT_CONNECTED", 0, {});
        }
    }
